...
async function signInAndAddToCart() {
    ...
    try {
        ...
        // Place Order
        let placeOrderButton = await driver.wait(until.elementLocated(By.xpath("//button[@type='submit' and contains(@class, 'action') and contains(@class, 'primary') and contains(@class, 'checkout') and @title='Place Order']")), 10000);
        await driver.wait(until.elementIsVisible(placeOrderButton), 15000);
    
        console.log("PLACE ORDER");

        // Continue Shopping
        let continueShoppingLink = await driver.wait(until.elementLocated(By.xpath("//a[@class='action primary continue' and contains(@href, 'magento.softwaretestingboard.com')]/span[text()='Continue Shopping']")), 10000);
